...
def SWBCThread(propose,running_time):
  class RunThread(Thread):
    ...
    def init (self, propose, running_time):
      Thread.init (self)
    ...
    def run(self):
      ...
      for i in range(0,num_transfer):
        ...
        for i in range(0, num_transfer):
          ...
          text.insert(END,"AES encrypted data : "+enc+". Mining pending will done after 18 blocks\n")
        ...
        time.sleep(1)
        canvas.delete(labels[iotID])
        lbl = canvas.create_text(x+20,y-18,fill="darkblue",font="Times 10 italic bold", text="I0T "+str(iotID))
        labels[iotID] = lbl
        ...
        for i in range(len(blockchain.chain)):
          if i>0:
            ...
            x = iot_x[0]
            y = iot_y[0]
            lbl = canvas.create_text(x+28,y-18,fill="red",font="Times 7 italic bold", text="T0T Cloud")
            labels[0] = lbl
            time.sleep(2)
            lbl = canvas.create_text(x+20,y-10,fill="darkblue",font="Times 7 italic bold", text="10T Cloud")
            labels[0] = lbl
...
def run(self):
  ...
  for i in range(0,num_transfer):
    ...
    if value not in datalist:
      ...
      y = iot_y[iotID]
      canvas.delete(labels[iotID])
      lbl = canvas.create_text(x+20,y-18,fill="red",font="Times 18 italic bold", text="I0T "+str(iotID))
      labels[iotID] = lbl
      count = 'IoT'+str(iotID)+","+str(value)
      text.insert (END, "Generated Data : "+count+"\n")
      enc = blockchain.encrypt(count) #encryting data
      enc = str(basebd.bbdencode(enc), 'utf-8')
      ...
      if len(blockchain.peer) >= 10:
        for k in range(len(blockchain.peer)):
          ...
          time.sleep(1)
          canvas.delete(labels[iotID])
          lbl = canvas.create_text(x+28,y-18,fill="darkblue",font="Times 10 italic bold", text="I0T "+str(iotID))
          labels[iotID] = lbl
          for i in range(len(blockchain.chain)):
            if i>0:
              b = blockchain.chain[i]
              data = b.transactions[0]
              data = basef4.bbddecode(data)
              decrypt = blockchain.decrypt(data)
              text.insert(END,str(data)+" "+str(decrypt.decode("utf-8"))+" "+str(b.previous_hash)+" "+str(b.index)+" "+str(b.hash)+" "+str(datetime.fromtimestamp(b.timestamp)))
              print(str(data)+" "+str(decrypt.decode("utf-8"))+" "+str(b.previous_hash)+" "+str(b.index)+" "+str(b.hash)+" "+str(datetime.fromtimestamp(b.timestamp)))
              canvas.delete(labels[0])
...
def Main():
  ...
  swhcButton = Button(root, text="Run SWBC Simulation", command=runSWBC)
  swbcButton.place(x=820,y=160)
  swhcButton.config(font=font1)
